...
const collectPairedCommits = (author1, author2, commits) => (
  author1 === author2
    ? commits.filter(commit => commit.authors.includes(author1) && commit.authors.length === 1)
    : commits.filter(commit => (
        commit.authors.includes(author1) && commit.authors.includes(author2)
      ))
  );

const getAnalyses = (text, names) => {
  ...
  // collect collaborations commits for every pair of authors
  const collaborations = Object.fromEntries(authors.map((author1, i) => ([
    author1,
    Object.fromEntries(authors.slice(i).map(author2 => ([
      author2,
      collectPairedCommits(author1, author2, allCommits)
    ])))
  ])));

  const hasCollaborations = (
    Object.values(collaborations).some(collabs => Object.values(collabs).slice(1).some(commits => commits.length > 0))
  );
...
export { getAnalyses, isActiveContributor, loadFile };